// Boost actually has two different state machine libraries
...
// using actual types as opposed to just enums
// transitions/events
struct CallDialed {};
struct HungUp {};
...
// in order to build the state machine, you have to inherit from a particular class
// going to take a look at some particular customizations, like getting additional behaviors out of states
struct PhoneStateMachine : state_machine_def<PhoneStateMachine>
{
    ...
    {
        // defining funciton, handling the situation where you enter the particular state
        template <class Event, class FSM>
        void on_entry(Event const& evt, FSM&) // its not an override
        {
    ...
    // table is kind of similar to the map that we've been defining by hand, (uses Boost EMPL, the metaprogramming library.)
    // start, event, target, action, guard
    // transition from on hold to fund this joint can only happen if the guard condition,  which is called can destroy phone, 
    // is actually allowing this sort of thing to happen.
